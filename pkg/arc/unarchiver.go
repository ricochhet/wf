...
func securePath(basePath, relativePath string) (string, error) {
	relativePath = filepath.Clean(
		"/" + relativePath,
	) // Normalize path with a leading slash
	...
	if !strings.HasPrefix(
		filepath.Clean(dstPath)+string(os.PathSeparator),
		filepath.Clean(basePath)+string(os.PathSeparator),
	) {
...
func createDirWithPermissions(path string, mode os.FileMode) error {
	logf("Creating directory: %s", path)
...
func handleFile(f archives.FileInfo, dst string) error {
	logf("Handling file: %s", f.NameInArchive)
	...
	if f.IsDir() {
		...
		logf("Successfully created directory: %s", dstPath)
	...
	if f.LinkTarget != "" {
		logf("Skipping symlink: %s -> %s", dstPath, f.LinkTarget)
		return nil
	}
	...
	if originalMode.Mode().Perm()&0o200 == 0 {
		logf("Parent directory is read-only, temporarily making it writable: %s", parentDir)
		...
		defer func() {
			...
			if chmodErr := os.Chmod(parentDir, originalMode.Mode()); chmodErr != nil {
				logf("Failed to restore original permissions for %s: %v", parentDir, chmodErr)
			}
		}()
	}
	...
	logf("Successfully extracted file: %s", dstPath)
...
func Unarchive(tarball, dst string) error {
	logf("Unarchiving %s to %s", tarball, dst)
	...
	if !ok {
		return errors.New("unsupported format for extraction")
	}
	...
	logf("Unarchiving completed successfully.")
